...
test("empty styled without params", () => {
	expect(() => styled()
		``()).toThrowError("Wrong extended element");
});
...
test("styled with simple css", () => {
	expect(styled.button `color: red;`()).toBe("color: red;");
});
...
test("styled with query", () => {
	const styledColor = styled.button `color: ${(argument) => argument.color || "red"};`;
	...
	expect(styledColor()).toBe("color: red;");
});
...
test("styled with query multiline", () => {
	...
	expect(styledColor()).toBe("color: red;background: white;");
});

test("extended styled", () => {
	const styledColor = styled.a `color: ${(argument) => argument.color || "red"};`;
	...
	expect(styledColorWithBackground()).toBe("color: red;background: white;");
});

test("mixin", () => {
	const textAlign = css `text-align: ${(argument) => argument.align || "center"};`;

	const font = css `font-family: Arial;`;
...
test("extended styled without duplication", () => {
	const styledColor = styled.a `color: black;`;

	const extendedStyledColor = styled(styledColor)
	`
		${(argument) => argument.color && css`color: ${argument.color};`};
	`;

	expect(extendedStyledColor()).toBe("color: black;");
